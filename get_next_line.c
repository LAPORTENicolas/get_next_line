...
char	*realloc_char(t_file *file, int linebreak)
{
	...
	k = ft_strlen(file->line, 0);
	new = ft_give_malloc(linebreak, k + ft_strlen(&file->buf[file->index], 1));
	if (new == NULL)
		return (NULL);
	i = 0;
	j = file->index;
	...
	while (j - file->index < ft_strlen(&file->buf[file->index], 1))
		new[i++] = file->buf[j++];
	file->index = j;
	if (file->line != NULL)
		free(file->line);
